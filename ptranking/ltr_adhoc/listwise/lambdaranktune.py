...
class LambdaRankTune(AdhocNeuralRanker):
    ...
    def ini_pointsf(self, num_features=None, h_dim=100, out_dim=100, num_layers=3, AF='R', TL_AF='S', apply_tl_af=False,
        ...
        # point_sf.add_module('_'.join(['project', 'linear', str(1)]), nn.Linear(h_dim, h_dim))
        # point_sf.add_module('_'.join(['project', 'relu', str(1)]), nn.ReLU())
        # for i in range(self.probe_layers - 1):
        #     # point_sf.add_module('_'.join(['ff', 'scoring', 'dr', str(i)]), nn.Dropout(dropout))
        #     point_sf.add_module('_'.join(['ff', 'scoring', str(i), 'batch_norm']), LTRBatchNorm(h_dim, momentum=0.1, affine=bn_affine, track_running_stats=False))
        #     point_sf.add_module('_'.join(['ff', 'scoring', str(i), 'ReLU']), nn.ReLU())
        #     nr_hn = nn.Linear(h_dim, h_dim)
        #     point_sf.add_module('_'.join(['ff', 'scoring', str(i)]), nr_hn)
        nr_hn = nn.Linear(h_dim, h_dim)
        point_sf.add_module('_'.join(['ff', 'scoring', '1']), nr_hn)
        point_sf.add_module('_'.join(['relu', 'scoring', '1']), nr_hn)
